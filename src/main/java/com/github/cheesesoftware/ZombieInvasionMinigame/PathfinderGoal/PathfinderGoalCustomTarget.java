...
public abstract class PathfinderGoalCustomTarget extends PathfinderGoal {
    ...
    protected double f() {
        AttributeInstance attributeinstance = this.entity.getAttributeInstance(GenericAttributes.FOLLOW_RANGE);
    ...
    protected boolean canTarget(EntityLiving target, boolean flag) // canExecute
    {
        if (target == null)
            ...
            return false;
        else if (!this.entity.f(new BlockPosition(target)))
            return false;
        else {
            if (this.entity instanceof EntityOwnable && StringUtils.isNotEmpty(((EntityOwnable) this.entity).getOwnerUUID().toString())) {
