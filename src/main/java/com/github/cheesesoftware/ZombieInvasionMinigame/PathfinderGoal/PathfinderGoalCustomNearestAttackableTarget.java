...
public class PathfinderGoalCustomNearestAttackableTarget extends PathfinderGoalCustomTarget {
    ...
    public boolean a() // canExecute
    {
        if (this.attackCheckFrequency > 0 && r.nextInt(this.attackCheckFrequency) != 0) {
            return false;
        } else if (state != null && (this.entity.getGoalTarget() == null
                || (this.entity.getGoalTarget().getBukkitEntity() instanceof Player && state.isSpectator((Player) this.entity.getGoalTarget().getBukkitEntity()))))
