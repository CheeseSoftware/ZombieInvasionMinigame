...
public class PathfinderGoalBreakBlock extends PathfinderGoal {
    ...
    private static List<Material> nonBreakableMaterials = new ArrayList<Material>(Arrays.asList(Material.BEDROCK, Material.getMaterial(8), Material.getMaterial(9), Material.GRASS, Material.SAND,
	    Material.AIR, Material.QUARTZ_BLOCK, Material.STONE, Material.WOOD_STAIRS, Material.BIRCH_WOOD_STAIRS, Material.BRICK_STAIRS, Material.COBBLESTONE_STAIRS, Material.JUNGLE_WOOD_STAIRS,
	    Material.NETHER_BRICK_STAIRS, Material.QUARTZ_STAIRS, Material.SANDSTONE_STAIRS, Material.SMOOTH_STAIRS, Material.SPRUCE_WOOD_STAIRS));
	...
	if (currentLocation.getBlockX() == oldLocation.getBlockX() && currentLocation.getBlockY() == oldLocation.getBlockY() && currentLocation.getBlockZ() == oldLocation.getBlockZ()) {
		...
		if (breakEnergy >= hardness) {
		    float totalDamage = BetterBlockBreaking.getPlugin().getDamageBlock(block.getLocation()).getDamage() + ((float) (2 * (breakEnergy / hardness)) / 240.0F * 10.0F);
		    BetterBlockBreaking.getPlugin().getDamageBlock(block.getLocation()).setDamage(totalDamage, null);
		...
		/*
		 * if (breakEnergy >= hardness) { this.i += 2 * (breakEnergy / hardness); breakEnergy %= hardness;
		 * 
		 * int i = (int) ((float) this.i / 240.0F * 10.0F); // i += (int) BetterBlockBreaking.getPlugin().getDamageBlock(block.getLocation()).getDamage(); float amountDamaged = i - j; float
		 * totalDamage = BetterBlockBreaking.getPlugin().getDamageBlock(block.getLocation()).getDamage() + amountDamaged;
		 * 
		 * if (i != this.j) { // this.entity.world.c(this.entity.getId(), new BlockPosition(block.getX(), block.getY(), block.getZ()), i);
		 * BetterBlockBreaking.getPlugin().getDamageBlock(block.getLocation()).setDamage(totalDamage, null); this.j = i; }
		 * 
